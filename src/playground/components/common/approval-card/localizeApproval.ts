import type { TFunction } from '@/playground/i18n'

/**
 * Approval copy (title / description / impact) is authored in English by
 * the plugins that raise the approval; we localize it here, inside the card.
 * Each string is looked up in a per-locale phrase map shipped in the locale
 * files under `chat.approval.phrases`.
 *
