...
- [Wolffish Cloud](https://wolffi.sh/cloud): the edition for regulated Saudi enterprises, an in-perimeter agentic workforce platform. One persistent agent per employee on the employee's own machine, inference on a zero-retention endpoint on the customer's own account (DeepInfra by default, ALLaM on a HUMAIN in-Kingdom endpoint supported), an action-level audit log written to the customer's own store, hard token quotas, SSO, and first-class integrations into internal and legacy systems deployed by the engineers who built it. Published pricing: SAR 60,000 deployment once, SAR 79 per seat per month, plus SAR 45 to 55 per seat per month billed by the customer's own providers at zero margin; landed cost SAR 124 to 184 per employee per month excluding VAT. For Saudi-owned regulated enterprises of 200 to 600 knowledge workers; six to eight weeks to a pilot cohort; Kingdom of Saudi Arabia only
